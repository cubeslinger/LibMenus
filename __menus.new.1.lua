...
-- 						if next(self.submenu)	==	nil then self.submenu[menuid] = {}  end
						if self.submenu[self.menuid]	==	nil then self.submenu[self.menuid] = {}  end

						table.insert(self.submenu[self.menuid], { [tbl.name] = {} })

						print("Menu call to SUB Menu ---------------------BEGIN---")
						print(string.format("menuid=%s self.submenu[self.menuid]=%s tbl.name=%s", self.menuid, self.submenu[self.menuid], tbl.name))
...
-- 						self.submenu[self.menuid][tbl.name]  =  new(o.text, submenuid, tbl.submenu)
						self.submenu[self.menuid][tbl.name]  =  new(o.text, tbl.submenu)
...
-- 														__menus.f.dumptable(self.submenu[self.menuid][tbl.name])
														self.submenu[self.menuid][tbl.name]:flip()
													end,
